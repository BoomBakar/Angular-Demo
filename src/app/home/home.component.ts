  ...
  template: `
    <section class="home">
     <form>
      ...
      <button class="primary-button" type="button" (click)="filterResults(filter.value)">Search</button> 
      </form>
    </section>
  ...
  constructor() {
    ...
    filterResults(filter: string) {
      ...
      this.filteredLocationList = this.housingLocationList.filter((housingLocation: HousingLocation) => {
        return housingLocation.name.toLowerCase().includes(filter.toLowerCase());
      })
    }
}
